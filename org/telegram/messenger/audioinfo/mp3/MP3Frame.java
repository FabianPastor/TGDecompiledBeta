...
public class MP3Frame
{
  ...
  public int getNumberOfFrames()
  {
    ...
    if (isXingFrame())
    {
      ...
      i = (this.bytes[(i + 8)] & 0xFF) << 24 | (this.bytes[(i + 9)] & 0xFF) << 16 | (this.bytes[(i + 10)] & 0xFF) << 8 | this.bytes[(i + 11)] & 0xFF;
    }
    for (;;)
    {
      return i;
      if (isVBRIFrame())
      {
        ...
        i = (this.bytes[(i + 14)] & 0xFF) << 24 | (this.bytes[(i + 15)] & 0xFF) << 16 | (this.bytes[(i + 16)] & 0xFF) << 8 | this.bytes[(i + 17)] & 0xFF;
      }
      else
      {
        label179:
  ...
  boolean isChecksumError()
  {
    ...
    if ((this.header.getProtection() == 0) && (this.header.getLayer() == 1))
    {
      ...
      int i = this.header.getSideInfoSize();
      for (int j = 0; j < i; j++) {
        localCRC16.update(this.bytes[(j + 6)]);
      }
      if (((this.bytes[4] & 0xFF) << 8 | this.bytes[5] & 0xFF) == localCRC16.getValue()) {}
    }
    for (;;)
    {
      return bool;
  ...
  boolean isVBRIFrame()
  {
    ...
    for (;;)
    {
      ...
      {
        bool2 = bool1;
        if (this.bytes[(i + 1)] == 66)
        {
          bool2 = bool1;
          if (this.bytes[(i + 2)] == 82)
          {
            bool2 = bool1;
            if (this.bytes[(i + 3)] == 73) {
              bool2 = true;
            }
          }
        }
      ...
      if (i >= 0)
      {
        ...
        if (this.bytes.length >= i + 8) {
          ...
          else
          {
            ...
            {
              bool2 = bool1;
              if (this.bytes[(i + 1)] == 110)
              {
                bool2 = bool1;
                if (this.bytes[(i + 2)] == 102)
                {
                  bool2 = bool1;
                  if (this.bytes[(i + 3)] == 111) {
                    bool2 = true;
                  }
                }
              }
  ...
  static final class CRC16
  {
    ...
    public void update(int paramInt1, int paramInt2)
    {
      ...
      label75:
      label81:
      for (this.crc = ((short)(short)(this.crc ^ 0x8005));; this.crc = ((short)(short)(this.crc << 1)))
      {
        i = paramInt2 >>> 1;
        paramInt2 = i;
        if (i != 0) {
  ...
  public static class Header
  {
    ...
    private static final int[][] FREQUENCIES = { { 11025, -1, 22050, 44100 }, { 12000, -1, 24000, 48000 }, { 8000, -1, 16000, 32000 }, { -1, -1, -1, -1 } };
    ...
    static
    {
      int[] arrayOfInt1 = { 128000, 64000, 56000, 64000, 32000 };
      int[] arrayOfInt2 = { 160000, 80000, 64000, 80000, 40000 };
      int[] arrayOfInt3 = { 192000, 96000, 80000, 96000, 48000 };
      BITRATES = new int[][] { { 0, 0, 0, 0, 0 }, { 32000, 32000, 32000, 32000, 8000 }, { 64000, 48000, 40000, 48000, 16000 }, { 96000, 56000, 48000, 56000, 24000 }, arrayOfInt1, arrayOfInt2, arrayOfInt3, { 224000, 112000, 96000, 112000, 56000 }, { 256000, 128000, 112000, 128000, 64000 }, { 288000, 160000, 128000, 144000, 80000 }, { 320000, 192000, 160000, 160000, 96000 }, { 352000, 224000, 192000, 176000, 112000 }, { 384000, 256000, 224000, 192000, 128000 }, { 416000, 320000, 256000, 224000, 144000 }, { 448000, 384000, 320000, 256000, 160000 }, { -1, -1, -1, -1, -1 } };
    ...
    {
      if ((this.layer == paramHeader.layer) && (this.version == paramHeader.version) && (this.frequency == paramHeader.frequency) && (this.channelMode == paramHeader.channelMode)) {}
