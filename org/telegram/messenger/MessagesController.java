...
public class MessagesController implements NotificationCenterDelegate {
                    ...
                    public void run() {
                        ...
                        if (arrayList != null) {
                            for (int a = 0; a < arrayList.size(); a++) {
                                ...
                                if ((encryptedChat instanceof TL_encryptedChat) && AndroidUtilities.getMyLayerVersion(encryptedChat.layer) < 66) {
            ...
            if (ttl > 0 || ttl == Integer.MIN_VALUE) {
                ...
                if (((int) dialog_id) == 0) {
                    ...
                    if (chat != null) {
                        ...
                        if (ttl > 0) {
                            int time = ConnectionsManager.getInstance().getCurrentTime();
                            MessagesStorage.getInstance().createTaskForSecretChat((long) chat.id, time, time, 0, random_ids);
                        }
                    }
                }
            }
    ...
    public void markDialogAsRead(long dialog_id, int max_id, int max_positive_id, int max_date, boolean was, boolean popup) {
        ...
        if (lower_part != 0) {
            ...
            if (chat.ttl > 0 && was) {
                i2 = Math.max(ConnectionsManager.getInstance().getCurrentTime(), max_date);
                MessagesStorage.getInstance().createTaskForSecretChat((long) chat.id, i2, i2, 0, null);
            }
        }
    }
    ...
    public boolean processUpdateArray(ArrayList<Update> updates, ArrayList<User> usersArr, ArrayList<Chat> chatsArr, boolean fromGetDifference) {
        ...
        if (!tasks.isEmpty()) {
            for (a = 0; a < tasks.size(); a++) {
                ...
                MessagesStorage.getInstance().createTaskForSecretChat((long) update2.chat_id, update2.max_date, update2.date, 1, null);
            }
        }
        return true;
    }
