        ...
        if (VERSION.SDK_INT >= 24 && allPhotosAlbumEntry != null) {
            Utilities.globalQueue.postRunnable(new Runnable() {
                ...
                public void run() {
                    ...
                    if (MediaController.allPhotosAlbumEntry.photos.size() != count) {
                        if (MediaController.refreshGalleryRunnable != null) {
                            AndroidUtilities.cancelRunOnUIThread(MediaController.refreshGalleryRunnable);
                            MediaController.refreshGalleryRunnable = null;
                        }
                    ...
                    AndroidUtilities.runOnUIThread(new Runnable() {
                        public void run() {
                            ...
                            if (duration > 700) {
                                ...
                                if (send == 1) {
                                    SendMessagesHelper.getInstance().sendMessage(audioToSend, null, recordingAudioFileToSend.getAbsolutePath(), MediaController.this.recordDialogId, MediaController.this.recordReplyingMessageObject, null, null, 0);
                                }
                                NotificationCenter instance = NotificationCenter.getInstance();
