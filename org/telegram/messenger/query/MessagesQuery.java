...
public class MessagesQuery {
    ...
    private static void savePinnedMessage(final Message result) {
        MessagesStorage.getInstance().getStorageQueue().postRunnable(new Runnable() {
            public void run() {
                try {
    ...
    private static MessageObject broadcastPinnedMessage(Message result, ArrayList<User> users, ArrayList<Chat> chats, boolean isCache, boolean returnValue) {
        ...
        AndroidUtilities.runOnUIThread(new Runnable() {
            public void run() {
                ...
                NotificationCenter.getInstance().postNotificationName(NotificationCenter.didLoadedPinnedMessage, new MessageObject(message, usersDict, chatsDict, false));
            }
        });
        return null;
    }
