...
public class SharedMediaQuery
{
  ...
  public static void loadMedia(final long paramLong, int paramInt1, int paramInt2, final int paramInt3, boolean paramBoolean, final int paramInt4)
  {
    ...
    for (;;)
    {
      ...
      paramInt1 = ConnectionsManager.getInstance().sendRequest(localTL_messages_search, new RequestDelegate()
      {
        public void run(TLObject paramAnonymousTLObject, TLRPC.TL_error paramAnonymousTL_error)
        {
          ...
          for (;;)
          {
            SharedMediaQuery.processLoadedMedia(paramAnonymousTLObject, paramLong, this.val$count, paramInt3, paramInt4, false, bool, this.val$isChannel, bool);
            return;
            label73:
            bool = true;
          }
  ...
  private static void loadMediaDatabase(final long paramLong, int paramInt1, int paramInt2, final int paramInt3, final int paramInt4, final boolean paramBoolean)
  {
    MessagesStorage.getInstance().getStorageQueue().postRunnable(new Runnable()
    {
      public void run()
      {
        ...
        for (;;)
        {
          ...
          {
            SharedMediaQuery.processLoadedMedia(localTL_messages_messages, paramLong, this.val$count, paramBoolean, paramInt4, true, this.val$classGuid, paramInt3, false);
          }
          ...
          else
          {
            ...
            else
            {
              ...
              if (!localArrayList2.contains(Integer.valueOf(-((TLRPC.Message)localObject5).from_id)))
              {
                ...
                for (;;)
                {
                  SharedMediaQuery.processLoadedMedia(localTL_messages_messages, paramLong, this.val$count, paramBoolean, paramInt4, true, this.val$classGuid, paramInt3, bool);
                  return;
                }
                label1253:
                l2 = l1;
  ...
  public static void loadMusic(long paramLong1, long paramLong2)
  {
    MessagesStorage.getInstance().getStorageQueue().postRunnable(new Runnable()
    {
      public void run()
      {
        ...
        try
        {
          ...
          {
            SQLiteCursor localSQLiteCursor1 = MessagesStorage.getInstance().getDatabase().queryFinalized(String.format(Locale.US, "SELECT data, mid FROM media_v2 WHERE uid = %d AND mid < %d AND type = %d ORDER BY date DESC, mid DESC LIMIT 1000", new Object[] { Long.valueOf(this.val$uid), Long.valueOf(this.val$max_id), Integer.valueOf(4) }), new Object[0]);
        ...
        {
          return;
          SQLiteCursor localSQLiteCursor2 = MessagesStorage.getInstance().getDatabase().queryFinalized(String.format(Locale.US, "SELECT data, mid FROM media_v2 WHERE uid = %d AND mid > %d AND type = %d ORDER BY date DESC, mid DESC LIMIT 1000", new Object[] { Long.valueOf(this.val$uid), Long.valueOf(this.val$max_id), Integer.valueOf(4) }), new Object[0]);
          break;
  ...
  private static void processLoadedMedia(TLRPC.messages_Messages parammessages_Messages, final long paramLong, int paramInt1, int paramInt2, final int paramInt3, final boolean paramBoolean1, final int paramInt4, boolean paramBoolean2, final boolean paramBoolean3)
  {
    int i = (int)paramLong;
    ...
    AndroidUtilities.runOnUIThread(new Runnable()
    {
      public void run()
      {
        ...
        NotificationCenter.getInstance().postNotificationName(NotificationCenter.mediaDidLoaded, new Object[] { Long.valueOf(paramLong), Integer.valueOf(i), paramInt4, Integer.valueOf(paramInt3), Integer.valueOf(paramBoolean3), Boolean.valueOf(this.val$topReached) });
      }
    });
  }
