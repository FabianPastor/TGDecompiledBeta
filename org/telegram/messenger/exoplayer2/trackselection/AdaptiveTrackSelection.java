  ...
  extends BaseTrackSelection
{
  ...
  private int determineIdealSelectedIndex(long paramLong)
  {
    ...
    if (l == -1L)
    {
      ...
      if ((paramLong != Long.MIN_VALUE) && (isBlacklisted(j, paramLong))) {
        break label100;
      }
      if (Math.round(getFormat(j).bitrate * this.playbackSpeed) > l) {
        break label96;
      }
    }
    for (;;)
  ...
  private long minDurationForQualityIncreaseUs(long paramLong)
  {
    ...
    label39:
    for (paramLong = ((float)paramLong * this.bufferedFractionToLiveEdgeForQualityIncrease);; paramLong = this.minDurationForQualityIncreaseUs)
    {
      return paramLong;
      i = 0;
      break;
  ...
  public int evaluateQueueSize(long paramLong, List<? extends MediaChunk> paramList)
  {
    ...
    do
    {
      ...
      j = paramList.size();
      i = j;
    } while (Util.getPlayoutDurationForMediaDuration(((MediaChunk)paramList.get(j - 1)).startTimeUs - paramLong, this.playbackSpeed) < this.minDurationToRetainAfterDiscardUs);
    ...
    for (;;)
    {
      ...
      for (this.selectedIndex = i;; this.selectedIndex = i) {
        do
        {
          ...
          this.reason = 3;
          break;
        } while ((localFormat2.bitrate >= localFormat1.bitrate) || (paramLong2 < this.maxDurationForQualityDecreaseUs));
      }
    }
  }
...
/* Location:              /home/fabian/Escritorio/Proyectos/Android/Dev/jardecompiler/TMessagesProj-fat-debug.jar!/org/telegram/messenger/exoplayer2/trackselection/AdaptiveTrackSelection.class
