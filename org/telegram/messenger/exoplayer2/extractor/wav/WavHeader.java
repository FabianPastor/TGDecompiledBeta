...
final class WavHeader implements SeekMap {
    ...
    public SeekPoints getSeekPoints(long timeUs) {
        long positionOffset = Util.constrainValue((((((long) this.averageBytesPerSecond) * timeUs) / C.MICROS_PER_SECOND) / ((long) this.blockAlignment)) * ((long) this.blockAlignment), 0, this.dataSize - ((long) this.blockAlignment));
    ...
    public long getTimeUs(long position) {
        return (C.MICROS_PER_SECOND * Math.max(0, position - this.dataStartPosition)) / ((long) this.averageBytesPerSecond);
    }
