...
abstract class Atom
{
    ...
    extends Atom
  {
    ...
    public int getChildAtomOfTypeCount(int paramInt)
    {
      ...
      {
        m = i;
        if (((Atom.LeafAtom)this.leafChildren.get(k)).type == paramInt) {
          m = i + 1;
        }
        k++;
        i = m;
      }
      j = this.containerChildren.size();
      k = 0;
      for (int m = i; k < j; m = i)
      {
        i = m;
        if (((ContainerAtom)this.containerChildren.get(k)).type == paramInt) {
          i = m + 1;
        }
        k++;
      }
    ...
    {
      int i = this.containerChildren.size();
      int j = 0;
      ContainerAtom localContainerAtom;
      if (j < i)
      {
        localContainerAtom = (ContainerAtom)this.containerChildren.get(j);
      ...
      for (;;)
      {
        ...
        j++;
        break;
        localContainerAtom = null;
      }
    }
    
    public Atom.LeafAtom getLeafAtomOfType(int paramInt)
    {
      int i = this.leafChildren.size();
      int j = 0;
      Atom.LeafAtom localLeafAtom;
      if (j < i)
      {
        localLeafAtom = (Atom.LeafAtom)this.leafChildren.get(j);
...
/* Location:              /home/fabian/Escritorio/Proyectos/Android/Dev/jardecompiler/TMessagesProj-fat-debug.jar!/org/telegram/messenger/exoplayer2/extractor/mp4/Atom.class
