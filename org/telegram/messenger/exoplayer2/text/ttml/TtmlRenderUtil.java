      ...
      do
      {
        do
        {
          for (;;)
          {
            ...
            {
              localTtmlStyle = (TtmlStyle)paramMap.get(paramArrayOfString[0]);
            }
            else if ((paramTtmlStyle == null) && (paramArrayOfString.length > 1))
            {
              localTtmlStyle = new TtmlStyle();
              i = paramArrayOfString.length;
              for (j = 0; j < i; j++) {
                localTtmlStyle.chain((TtmlStyle)paramMap.get(paramArrayOfString[j]));
              }
            }
            else
            {
              ...
              localTtmlStyle = paramTtmlStyle.chain((TtmlStyle)paramMap.get(paramArrayOfString[0]));
            }
          }
...
/* Location:              /home/fabian/Escritorio/Proyectos/Android/Dev/jardecompiler/TMessagesProj-fat-debug.jar!/org/telegram/messenger/exoplayer2/text/ttml/TtmlRenderUtil.class
