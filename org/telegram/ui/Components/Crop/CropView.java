...
public class CropView extends FrameLayout implements AreaViewListener, CropGestureListener {
    ...
    private class CropState {
        ...
        private CropState(Bitmap bitmap) {
        ...
        private float getOrientedWidth() {
            return this.orientation % BitmapDescriptorFactory.HUE_CYAN != 0.0f ? this.height : this.width;
        }

        private float getOrientedHeight() {
            return this.orientation % BitmapDescriptorFactory.HUE_CYAN != 0.0f ? this.width : this.height;
        }
        ...
        private void reset(CropAreaView areaView, float orient, boolean freeform) {
            ...
            float w = this.orientation % BitmapDescriptorFactory.HUE_CYAN != 0.0f ? this.height : this.width;
            float h = this.orientation % BitmapDescriptorFactory.HUE_CYAN != 0.0f ? this.width : this.height;
            if (freeform) {
    ...
    private class FitScaleResult {
        RectF contentRect;
        float scale;

        private FitScaleResult() {
        }
    }
    ...
    public void setBitmap(Bitmap b, boolean fform) {
        this.bitmap = b;
        this.freeform = fform;
        this.state = new CropState(this.bitmap);
        this.backView.setVisibility(4);
    ...
    public void reset() {
        this.areaView.setBitmap(this.bitmap, false, this.freeform);
    ...
    private void fillAreaView(RectF targetRect, boolean allowZoomOut) {
        ...
        final float y = ((targetRect.centerY() - ((float) (this.imageView.getHeight() / 2))) / this.areaView.getCropHeight()) * this.state.getOrientedHeight();
    ...
    private FitScaleResult fitScale(RectF contentRect, float scale, float ratio) {
        ...
        RectF targetContentRect = new RectF(contentRect.left + scaledX, contentRect.top + scaledY, (contentRect.left + scaledX) + scaledW, (contentRect.top + scaledY) + scaledH);
        FitScaleResult result = new FitScaleResult();
        result.contentRect = targetContentRect;
        result.scale = scale * ratio;
        return result;
    }

    private PointF fitTranslation(RectF contentRect, RectF boundsRect, PointF translation, float radians) {
        ...
        RectF validBoundsRect = new RectF(frameLeft, frameTop, boundsRect.width() + frameLeft, boundsRect.height() + frameTop);
        float deltaX = boundsRect.centerX() - validBoundsRect.centerX();
        float deltaY = boundsRect.centerY() - validBoundsRect.centerY();
        return new PointF((translation.x + ((float) (Math.sin(1.5707963267948966d - ((double) radians)) * ((double) deltaX)))) + ((float) (Math.cos(1.5707963267948966d + ((double) radians)) * ((double) deltaY))), (translation.y + ((float) (Math.cos(1.5707963267948966d - ((double) radians)) * ((double) deltaX)))) + ((float) (Math.sin(1.5707963267948966d + ((double) radians)) * ((double) deltaY))));
    }
    ...
    private void fitContentInBounds(boolean allowScale, boolean maximize, boolean animated) {
        ...
        float centerOffsetX = (((this.initialAreaRect.centerX() - boundsRect.centerX()) - this.initialAreaRect.left) - this.initialAreaRect.right) + this.areaView.getCropRight();
        float centerOffsetY = (((this.initialAreaRect.centerY() - boundsRect.centerY()) - this.initialAreaRect.top) - this.initialAreaRect.bottom) + this.areaView.getCropBottom();
        float compX = (((float) Math.sin(1.5707963267948966d + ((double) radians))) * centerOffsetX) + (((float) Math.cos(1.5707963267948966d - ((double) radians))) * centerOffsetY);
        float compY = (((float) Math.cos(1.5707963267948966d + ((double) radians))) * centerOffsetX) + (((float) Math.sin(1.5707963267948966d - ((double) radians))) * centerOffsetY);
        this.tempRect.setRect(rectF);
        Matrix matrix = this.state.getMatrix();
        matrix.preTranslate((initialX + compX) / scale, (initialY + compY) / scale);
        ...
        FitScaleResult scaleResult;
        RectF contentRect;
        if (!rectF.contains(boundsRect)) {
            scaleResult = null;
            if (allowScale && (boundsRect.width() > rectF.width() || boundsRect.height() > rectF.height())) {
                scaleResult = fitScale(rectF, scale, boundsRect.width() / scaleWidthToMaxSize(boundsRect, rectF));
            }
            if (scaleResult != null) {
                contentRect = scaleResult.contentRect;
                targetScale = scaleResult.scale;
            }
            targetTranslation = fitTranslation(contentRect, boundsRect, pointF, radians);
            ...
            scaleResult = fitScale(rectF, scale, ratio);
            contentRect = scaleResult.contentRect;
            targetScale = scaleResult.scale;
            targetTranslation = fitTranslation(contentRect, boundsRect, pointF, radians);
        }
        ...
        if (animated) {
            ...
            animator.addUpdateListener(new AnimatorUpdateListener() {
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    float deltaX = (dx * value) - currentValues[1];
                    float[] fArr = currentValues;
                    fArr[1] = fArr[1] + deltaX;
                    float deltaY = (dy * value) - currentValues[2];
                    fArr = currentValues;
                    fArr[2] = fArr[2] + deltaY;
                    CropView.this.state.translate(deltaX, deltaY);
    ...
    public void rotate90Degrees() {
        boolean z = true;
        resetRotationStartScale();
        float orientation = (this.state.getOrientation() - 90.0f) % 360.0f;
        this.areaView.setBitmap(this.bitmap, orientation % BitmapDescriptorFactory.HUE_CYAN != 0.0f, this.freeform);
    ...
    public void onScale(float scale, float x, float y) {
        ...
        this.state.scale(scale, 0.0f, 0.0f);
        updateMatrix();
    }
