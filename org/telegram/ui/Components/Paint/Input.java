...
public class Input
{
  ...
  private void smoothenAndPaintPoints(boolean paramBoolean)
  {
    ...
    if (this.pointsCount > 2)
    {
      ...
      localPoint1 = this.points[0];
      localObject2 = this.points[1];
      localPoint2 = this.points[2];
    ...
    for (;;)
    {
      ...
      Point localPoint3 = localPoint2.multiplySum((Point)localObject2, 0.5D);
      int i = (int)Math.min(48.0D, Math.max(Math.floor(localPoint1.getDistanceTo(localPoint3) / 1), 24.0D));
      float f1 = 0.0F;
      float f2 = 1.0F / i;
      for (int j = 0; j < i; j++)
      {
        localPoint2 = smoothPoint(localPoint1, localPoint3, (Point)localObject2, f1);
        if (this.isFirst)
        {
      ...
      else
      {
        this.pointsCount = 2;
        continue;
        localObject1 = new Point[this.pointsCount];
  ...
  public void process(MotionEvent paramMotionEvent)
  {
    ...
    case 2: 
      for (;;)
      {
        ...
        else if (paramMotionEvent.getDistanceTo(this.lastLocation) >= AndroidUtilities.dp(5.0F))
        {
          if (!this.hasMoved)
          {
