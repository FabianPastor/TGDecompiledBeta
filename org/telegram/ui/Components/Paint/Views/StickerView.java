  ...
  extends EntityView
{
    ...
    extends EntityView.SelectionView
  {
    ...
    protected void onDraw(Canvas paramCanvas)
    {
      ...
      float f2 = AndroidUtilities.dp(4.5F);
      float f3 = f2 + f1 + AndroidUtilities.dp(15.0F);
      f1 = getWidth() / 2 - f3;
      this.arcRect.set(f3, f3, f1 * 2.0F + f3, f1 * 2.0F + f3);
      for (int i = 0; i < 48; i++) {
        paramCanvas.drawArc(this.arcRect, (4.0F + 4.0F) * i, 4.0F, false, this.arcPaint);
      }
      paramCanvas.drawCircle(f3, f3 + f1, f2, this.dotPaint);
      paramCanvas.drawCircle(f3, f3 + f1, f2, this.dotStrokePaint);
      paramCanvas.drawCircle(f1 * 2.0F + f3, f3 + f1, f2, this.dotPaint);
      paramCanvas.drawCircle(f1 * 2.0F + f3, f3 + f1, f2, this.dotStrokePaint);
    }
    ...
    protected int pointInsideHandle(float paramFloat1, float paramFloat2)
    {
      ...
      f1 = f2 + f1;
      float f3 = f1 + (getHeight() - f1 * 2.0F) / 2.0F;
      int i;
      if ((paramFloat1 > f1 - f2) && (paramFloat2 > f3 - f2) && (paramFloat1 < f1 + f2) && (paramFloat2 < f3 + f2)) {
        i = 1;
      }
      for (;;)
      {
        return i;
        if ((paramFloat1 > getWidth() - f1 * 2.0F + f1 - f2) && (paramFloat2 > f3 - f2) && (paramFloat1 < getWidth() - f1 * 2.0F + f1 + f2) && (paramFloat2 < f3 + f2))
        {
          i = 2;
        }
        else
        {
          f2 = getWidth() / 2.0F;
          if (Math.pow(paramFloat1 - f2, 2.0D) + Math.pow(paramFloat2 - f2, 2.0D) < Math.pow(f2, 2.0D)) {
            i = 3;
          } else {
            i = 0;
          }
