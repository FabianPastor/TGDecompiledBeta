      ...
      for (;;)
      {
        ...
        if (i != 0)
        {
          ...
          if (this.currentHandle == 3)
          {
            ...
            bool2 = EntityView.this.onTouchMove(f1, f2);
          }
          else
          {
            bool2 = bool1;
            if (this.currentHandle != 0)
            {
              ...
              f2 = (float)Math.toRadians(getRotation());
              f1 = (float)(localPoint.x * Math.cos(f2) + localPoint.y * Math.sin(f2));
              f2 = f1;
              if (this.currentHandle == 1) {
                f2 = f1 * -1.0F;
              }
              f2 = 2.0F * f2 / getWidth();
              EntityView.this.scale(1.0F + f2);
              float f3 = getLeft() + getWidth() / 2;
              f1 = getTop() + getHeight() / 2;
              ...
              f2 = 0.0F;
              if (this.currentHandle == 1) {
                f2 = (float)Math.atan2(f1 - f5, f3 - f4);
              }
              for (;;)
              {
                EntityView.this.rotate((float)Math.toDegrees(f2));
                EntityView.access$702(EntityView.this, paramMotionEvent.getRawX());
                EntityView.access$802(EntityView.this, paramMotionEvent.getRawY());
                bool2 = true;
                break;
                if (this.currentHandle == 2) {
                  f2 = (float)Math.atan2(f5 - f1, f4 - f3);
                }
              }
