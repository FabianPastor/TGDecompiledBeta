  ...
  extends Drawable
{
  ...
  public void draw(Canvas paramCanvas)
  {
    ...
    for (;;)
    {
      ...
      {
        i = 0;
        f2 = (float)Math.floor(Math.min(getBounds().width(), getBounds().height()) / 12.0F);
        f3 = Math.max(0.0F, (getBounds().width() - 12.0F * f2) / 2.0F);
        f1 = Math.max(0.0F, (getBounds().height() - 12.0F * f2) / 2.0F);
        for (j = 0; j < 12; j++) {
          for (k = 0; k < 12; k++)
          {
            m = Math.abs(getBits(i));
            this.paint.setColor(this.colors[(m % 4)]);
            paramCanvas.drawRect(f3 + k * f2, j * f2 + f1, k * f2 + f3 + f2, j * f2 + f2 + f1, this.paint);
            i += 2;
          }
        }
      }
