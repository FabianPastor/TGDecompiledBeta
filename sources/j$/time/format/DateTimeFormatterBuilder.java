...
public final class DateTimeFormatterBuilder {
    ...
    static final class ReducedPrinterParser extends NumberPrinterParser {
        ...
        public /* synthetic */ void m0xdf3a601e(DateTimeParseContext context, long initialValue, int errorPos, int successPos, Chronology _unused) {
    ...
    static final class OffsetIdPrinterParser implements DateTimePrinterParser {
        ...
        private boolean parseNumber(int[] array, int arrayIndex, CharSequence parseText, boolean required) {
            ...
            int pos2 = pos + 1;
            int pos3 = parseText.charAt(pos);
            int pos4 = pos2 + 1;
            int pos5 = parseText.charAt(pos2);
            if (pos3 < 48 || pos3 > 57 || pos5 < 48 || pos5 > 57 || (value = ((pos3 - 48) * 10) + (pos5 - 48)) < 0 || value > 59) {
                return required;
            }
    ...
    static final class LocalizedOffsetIdPrinterParser implements DateTimePrinterParser {
        ...
        public int parse(DateTimeParseContext context, CharSequence text, int position) {
            ...
            if (this.style == TextStyle.FULL) {
                ...
                int pos7 = pos6 + 1;
                int pos8 = getDigit(charSequence, pos6);
                if (h1 >= 0 && pos8 >= 0) {
                    int pos9 = pos7 + 1;
                    if (charSequence.charAt(pos7) != 58) {
                        ...
                        int h2 = (h1 * 10) + pos8;
                        int h3 = pos9 + 1;
                        int m12 = getDigit(charSequence, pos9);
                        int pos10 = h3 + 1;
                        int pos11 = getDigit(charSequence, h3);
                        if (m12 < 0 || pos11 < 0) {
                            return position ^ -1;
                        }
                        int m2 = (m12 * 10) + pos11;
                        if (pos10 + 2 < end && charSequence.charAt(pos10) == ':') {
                            int s1 = getDigit(charSequence, pos10 + 1);
                            int s2 = getDigit(charSequence, pos10 + 2);
                            if (s1 >= 0 && s2 >= 0) {
                                s = (s1 * 10) + s2;
                                pos10 += 3;
                            }
            ...
            if (pos12 < end) {
                ...
                if (pos12 + 2 < end && charSequence.charAt(pos12) == ':' && pos12 + 2 < end && charSequence.charAt(pos12) == ':') {
                    int m13 = getDigit(charSequence, pos12 + 1);
