...
public class Switch extends View {
    ...
    protected void onDraw(Canvas canvas) {
        if (getVisibility() == 0) {
            ...
            canvas.drawCircle((float) tx, (float) ty, (float) AndroidUtilities.dp(8.0f), this.paint);
            if (this.iconDrawable != null) {
                ...
                canvas.drawLine((float) tx, (float) ty, (float) tx, (float) (ty - AndroidUtilities.dp(5.0f)), this.paint2);
                canvas.save();
                canvas.rotate(-90.0f * this.iconProgress, (float) tx, (float) ty);
                canvas.drawLine((float) tx, (float) ty, (float) (AndroidUtilities.dp(4.0f) + tx), (float) ty, this.paint2);
                canvas.restore();
            }
        }
    }
