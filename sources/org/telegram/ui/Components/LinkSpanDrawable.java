...
public class LinkSpanDrawable<S extends CharacterStyle> {
    ...
    public static class LinkCollector {
        ...
        public /* synthetic */ void m1108xCLASSNAME(LinkSpanDrawable link) {
        ...
        public /* synthetic */ void m1109xd5d9var_(LinkSpanDrawable link) {
            removeLink(link, false);
        }
    ...
    public static class LinksTextView extends TextView {
        ...
        public LinksTextView(Context context, LinkCollector customLinkCollector, Theme.ResourcesProvider resourcesProvider2) {
            super(context);
        ...
        public boolean onTouchEvent(MotionEvent event) {
            if (this.links != null) {
                ...
                if (left > ((float) x) || textLayout.getLineWidth(line) + left < ((float) x) || y < 0 || y > textLayout.getHeight()) {
                    ...
                    ClickableSpan[] spans = (ClickableSpan[]) buffer.getSpans(off, off, ClickableSpan.class);
                    if (spans.length == 0 || AndroidUtilities.isAccessibilityScreenReaderEnabled()) {
                        ...
                        if (event.getAction() == 0) {
                            ...
                            path.setCurrentLayout(textLayout, start, (float) getPaddingTop());
                            textLayout.getSelectionPath(start, end, path);
                            int i = x;
                            AndroidUtilities.runOnUIThread(new LinkSpanDrawable$LinksTextView$$ExternalSyntheticLambda0(this, spans), (long) ViewConfiguration.getLongPressTimeout());
                            return true;
                        }
                    }
                }
            ...
            if (this.pressedLink != null || super.onTouchEvent(event)) {
                return true;
            }
            return false;
        }

        /* renamed from: lambda$onTouchEvent$0$org-telegram-ui-Components-LinkSpanDrawable$LinksTextView  reason: not valid java name */
        public /* synthetic */ void m1110x6CLASSNAMEbbe9(ClickableSpan[] spans) {
