...
public class PhotoFilterCurvesControl extends View {
    ...
    public void setActualArea(float x, float y, float width, float height) {
        this.actualArea.f70x = x;
        this.actualArea.f71y = y;
        ...
        switch (event.getActionMasked()) {
            ...
            case 5:
                if (event.getPointerCount() != 1) {
                    ...
                    if (locationX >= this.actualArea.f70x && locationX <= this.actualArea.f70x + this.actualArea.width && locationY >= this.actualArea.f71y && locationY <= this.actualArea.f71y + this.actualArea.height) {
                        this.isMoving = true;
                    }
    ...
    private void selectSegmentWithPoint(float pointx) {
        if (this.activeSegment == 0) {
            this.activeSegment = (int) Math.floor((double) (((pointx - this.actualArea.f70x) / (this.actualArea.width / 5.0f)) + 1.0f));
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < 4; i++) {
            canvas2 = canvas;
            canvas2.drawLine((((float) i) * segmentWidth) + (this.actualArea.f70x + segmentWidth), this.actualArea.f71y, (((float) i) * segmentWidth) + (this.actualArea.f70x + segmentWidth), this.actualArea.height + this.actualArea.f71y, this.paint);
        }
        canvas2 = canvas;
        canvas2.drawLine(this.actualArea.f70x, this.actualArea.height + this.actualArea.f71y, this.actualArea.width + this.actualArea.f70x, this.actualArea.f71y, this.paintDash);
        ...
        for (a = 0; a < 5; a++) {
            ...
            canvas.drawText(str, (this.actualArea.f70x + ((segmentWidth - this.textPaint.measureText(str)) / 2.0f)) + (((float) a) * segmentWidth), (this.actualArea.f71y + this.actualArea.height) - ((float) AndroidUtilities.dp(4.0f)), this.textPaint);
        }
        ...
        for (a = 0; a < points.length / 2; a++) {
            if (a == 0) {
                this.path.moveTo(this.actualArea.f70x + (points[a * 2] * this.actualArea.width), this.actualArea.f71y + ((1.0f - points[(a * 2) + 1]) * this.actualArea.height));
            } else {
                this.path.lineTo(this.actualArea.f70x + (points[a * 2] * this.actualArea.width), this.actualArea.f71y + ((1.0f - points[(a * 2) + 1]) * this.actualArea.height));
            }
        }
