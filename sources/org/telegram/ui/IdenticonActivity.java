...
public class IdenticonActivity extends BaseFragment implements NotificationCenterDelegate {
    ...
    class C14234 extends AnimatorListenerAdapter {
        ...
        public void onAnimationEnd(Animator animator) {
            if (animator.equals(IdenticonActivity.this.animatorSet) != null) {
    ...
    public View createView(Context context) {
        ...
        this.container = new FrameLayout(context) {
            protected void onLayout(boolean z, int i, int i2, int i3, int i4) {
                ...
                if (IdenticonActivity.this.codeTextView) {
                    z = (IdenticonActivity.this.codeTextView.getLeft() + (IdenticonActivity.this.codeTextView.getMeasuredWidth() / 2)) - (IdenticonActivity.this.emojiTextView.getMeasuredWidth() / 2);
                    i = (((IdenticonActivity.this.codeTextView.getMeasuredHeight() - IdenticonActivity.this.emojiTextView.getMeasuredHeight()) / 2) + IdenticonActivity.this.linearLayout1.getTop()) - AndroidUtilities.dp(NUM);
                    IdenticonActivity.this.emojiTextView.layout(z, i, IdenticonActivity.this.emojiTextView.getMeasuredWidth() + z, IdenticonActivity.this.emojiTextView.getMeasuredHeight() + i);
                }
            }
        };
        ...
        context = MessagesController.getInstance(this.currentAccount).getEncryptedChat(Integer.valueOf(this.chat_id));
        if (context != null) {
            Drawable identiconDrawable = new IdenticonDrawable();
            imageView.setImageDrawable(identiconDrawable);
            identiconDrawable.setEncryptedChat(context);
            User user = MessagesController.getInstance(this.currentAccount).getUser(Integer.valueOf(context.user_id));
            CharSequence spannableStringBuilder = new SpannableStringBuilder();
            StringBuilder stringBuilder = new StringBuilder();
            if (context.key_hash.length > 16) {
                int i;
                String bytesToHex = Utilities.bytesToHex(context.key_hash);
                for (i = 0; i < 32; i++) {
                    if (i != 0) {
                        if (i % 8 == 0) {
                ...
                for (int i3 = 0; i3 < 5; i3++) {
                    int i4 = (i3 * 4) + 16;
                    i = ((((context.key_hash[i4] & 127) << 24) | ((context.key_hash[i4 + 1] & 255) << 16)) | ((context.key_hash[i4 + 2] & 255) << 8)) | (context.key_hash[i4 + 3] & 255);
                    if (i3 != 0) {
                        stringBuilder.append(" ");
                    }
                    stringBuilder.append(EmojiData.emojiSecret[i % EmojiData.emojiSecret.length]);
                }
                this.emojiText = stringBuilder.toString();
            }
            this.codeTextView.setText(spannableStringBuilder.toString());
            spannableStringBuilder.clear();
            spannableStringBuilder.append(AndroidUtilities.replaceTags(LocaleController.formatString("EncryptionKeyDescription", C0446R.string.EncryptionKeyDescription, user.first_name, user.first_name)));
            context = spannableStringBuilder.toString().indexOf("telegram.org");
            if (context != -1) {
                spannableStringBuilder.setSpan(new URLSpanReplacement(LocaleController.getString("EncryptionKeyLink", C0446R.string.EncryptionKeyLink)), context, "telegram.org".length() + context, 33);
            }
    ...
    public void didReceivedNotification(int i, int i2, Object... objArr) {
        if (i == NotificationCenter.emojiDidLoaded && this.emojiTextView != 0) {
