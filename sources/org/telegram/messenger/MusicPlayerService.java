...
public class MusicPlayerService extends Service implements NotificationCenterDelegate {
    ...
    public void onCreate() {
        ...
        if (VERSION.SDK_INT >= 21) {
            ...
            Drawable placeholder = getResources().getDrawable(C0446R.drawable.nocover_big);
    ...
    private void createNotification(MessageObject messageObject) {
        ...
        if (VERSION.SDK_INT >= 21) {
            ...
            int[] iArr = new int[3];
            bldr.setSmallIcon(C0446R.drawable.player).setOngoing(isPlaying).setContentTitle(songName).setContentText(authorName).setSubText(audioInfo != null ? audioInfo.getAlbum() : null).setContentIntent(contentIntent).setDeleteIntent(pendingStop).setShowWhen(false).setCategory("transport").setPriority(2).setStyle(new MediaStyle().setMediaSession(this.mediaSession.getSessionToken()).setShowActionsInCompactView(new int[]{0, 1, 2}));
            ...
            if (MediaController.getInstance().isDownloadingCurrentMessage()) {
                ...
                bldr.addAction(new Action.Builder(C0446R.drawable.ic_action_previous, TtmlNode.ANONYMOUS_REGION_ID, pendingPrev).build()).addAction(new Action.Builder(C0446R.drawable.loading_animation2, TtmlNode.ANONYMOUS_REGION_ID, null).build()).addAction(new Action.Builder(C0446R.drawable.ic_action_next, TtmlNode.ANONYMOUS_REGION_ID, pendingNext).build());
            } else {
                ...
                bldr.addAction(new Action.Builder(C0446R.drawable.ic_action_previous, TtmlNode.ANONYMOUS_REGION_ID, pendingPrev).build()).addAction(new Action.Builder(isPlaying ? C0446R.drawable.ic_action_pause : C0446R.drawable.ic_action_play, TtmlNode.ANONYMOUS_REGION_ID, pendingPlaypause).build()).addAction(new Action.Builder(C0446R.drawable.ic_action_next, TtmlNode.ANONYMOUS_REGION_ID, pendingNext).build());
            }
            ...
            if (supportBigNotifications) {
                expandedView = new RemoteViews(getApplicationContext().getPackageName(), C0446R.layout.player_big_notification);
            }
            notification = new NotificationCompat.Builder(getApplicationContext()).setSmallIcon(C0446R.drawable.player).setContentIntent(contentIntent).setChannelId(NotificationsController.OTHER_NOTIFICATIONS_CHANNEL).setContentTitle(songName).build();
            ...
            albumArt = audioInfo != null ? audioInfo.getSmallCover() : null;
            if (albumArt != null) {
                notification.contentView.setImageViewBitmap(C0446R.id.player_album_art, albumArt);
                if (supportBigNotifications) {
                    notification.bigContentView.setImageViewBitmap(C0446R.id.player_album_art, albumArt);
                }
            } else {
                notification.contentView.setImageViewResource(C0446R.id.player_album_art, C0446R.drawable.nocover_small);
                if (supportBigNotifications) {
                    notification.bigContentView.setImageViewResource(C0446R.id.player_album_art, C0446R.drawable.nocover_big);
                }
            }
            if (MediaController.getInstance().isDownloadingCurrentMessage()) {
                notification.contentView.setViewVisibility(C0446R.id.player_pause, 8);
                notification.contentView.setViewVisibility(C0446R.id.player_play, 8);
                notification.contentView.setViewVisibility(C0446R.id.player_next, 8);
                notification.contentView.setViewVisibility(C0446R.id.player_previous, 8);
                notification.contentView.setViewVisibility(C0446R.id.player_progress_bar, 0);
                if (supportBigNotifications) {
                    notification.bigContentView.setViewVisibility(C0446R.id.player_pause, 8);
                    notification.bigContentView.setViewVisibility(C0446R.id.player_play, 8);
                    notification.bigContentView.setViewVisibility(C0446R.id.player_next, 8);
                    notification.bigContentView.setViewVisibility(C0446R.id.player_previous, 8);
                    notification.bigContentView.setViewVisibility(C0446R.id.player_progress_bar, 0);
                }
            } else {
                notification.contentView.setViewVisibility(C0446R.id.player_progress_bar, 8);
                notification.contentView.setViewVisibility(C0446R.id.player_next, 0);
                notification.contentView.setViewVisibility(C0446R.id.player_previous, 0);
                if (supportBigNotifications) {
                    notification.bigContentView.setViewVisibility(C0446R.id.player_next, 0);
                    notification.bigContentView.setViewVisibility(C0446R.id.player_previous, 0);
                    notification.bigContentView.setViewVisibility(C0446R.id.player_progress_bar, 8);
                }
                if (MediaController.getInstance().isMessagePaused()) {
                    notification.contentView.setViewVisibility(C0446R.id.player_pause, 8);
                    notification.contentView.setViewVisibility(C0446R.id.player_play, 0);
                    if (supportBigNotifications) {
                        notification.bigContentView.setViewVisibility(C0446R.id.player_pause, 8);
                        notification.bigContentView.setViewVisibility(C0446R.id.player_play, 0);
                    }
                } else {
                    notification.contentView.setViewVisibility(C0446R.id.player_pause, 0);
                    notification.contentView.setViewVisibility(C0446R.id.player_play, 8);
                    if (supportBigNotifications) {
                        notification.bigContentView.setViewVisibility(C0446R.id.player_pause, 0);
                        notification.bigContentView.setViewVisibility(C0446R.id.player_play, 8);
                    }
                }
            }
            notification.contentView.setTextViewText(C0446R.id.player_song_name, songName);
            notification.contentView.setTextViewText(C0446R.id.player_author_name, authorName);
            if (supportBigNotifications) {
                notification.bigContentView.setTextViewText(C0446R.id.player_song_name, songName);
                notification.bigContentView.setTextViewText(C0446R.id.player_author_name, authorName);
                ...
                remoteViews2.setTextViewText(C0446R.id.player_album_title, album);
            }
    ...
    public void setListeners(RemoteViews view) {
        view.setOnClickPendingIntent(C0446R.id.player_previous, PendingIntent.getBroadcast(getApplicationContext(), 0, new Intent(NOTIFY_PREVIOUS), 134217728));
        view.setOnClickPendingIntent(C0446R.id.player_close, PendingIntent.getBroadcast(getApplicationContext(), 0, new Intent(NOTIFY_CLOSE), 134217728));
        view.setOnClickPendingIntent(C0446R.id.player_pause, PendingIntent.getBroadcast(getApplicationContext(), 0, new Intent(NOTIFY_PAUSE), 134217728));
        view.setOnClickPendingIntent(C0446R.id.player_next, PendingIntent.getBroadcast(getApplicationContext(), 0, new Intent(NOTIFY_NEXT), 134217728));
        view.setOnClickPendingIntent(C0446R.id.player_play, PendingIntent.getBroadcast(getApplicationContext(), 0, new Intent(NOTIFY_PLAY), 134217728));
    }
