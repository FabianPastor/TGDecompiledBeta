...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> callback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C06801();
            ...
            class C06801 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem msg = C20011.this.mQueue.next();
                    while (msg != null) {
                        switch (msg.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> callback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C06811();
            ...
            class C06811 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem msg = C20022.this.mQueue.next();
                        if (msg != null) {
                            switch (msg.what) {
                                case 1:
                                    C20022.this.mQueue.removeMessages(1);
                                    callback.refresh(msg.arg1);
                                    break;
                                case 2:
                                    C20022.this.mQueue.removeMessages(2);
                                    C20022.this.mQueue.removeMessages(3);
                        ...
                        C20022.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
