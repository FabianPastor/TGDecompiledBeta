...
final class ExoPlayerImpl implements ExoPlayer {
    ...
    public void seekToDefaultPosition(int windowIndex) {
        seekTo(windowIndex, C0615C.TIME_UNSET);
    }
    ...
    public void seekTo(int windowIndex, long positionMs) {
        ...
        if (timeline.isEmpty()) {
            ...
            long windowPositionUs = positionMs == C0615C.TIME_UNSET ? timeline.getWindow(windowIndex, this.window).getDefaultPositionUs() : C0615C.msToUs(positionMs);
            Pair<Integer, Long> periodIndexAndPositon = timeline.getPeriodPosition(this.window, this.period, windowIndex, windowPositionUs);
            this.maskingWindowPositionMs = C0615C.usToMs(windowPositionUs);
            this.maskingPeriodIndex = ((Integer) periodIndexAndPositon.first).intValue();
        }
        this.internalPlayer.seekTo(timeline, windowIndex, C0615C.msToUs(positionMs));
        Iterator it = this.listeners.iterator();
        while (it.hasNext()) {
    ...
    public long getDuration() {
        ...
        return C0615C.usToMs(this.period.getAdDurationUs(periodId.adGroupIndex, periodId.adIndexInAdGroup));
    }
    ...
    public int getBufferedPercentage() {
        ...
        if (position == C0615C.TIME_UNSET || duration == C0615C.TIME_UNSET) {
            return 0;
        }
        if (duration != 0) {
    ...
    public long getContentPosition() {
        ...
        return this.period.getPositionInWindowMs() + C0615C.usToMs(this.playbackInfo.contentPositionUs);
    }
    ...
    private void handlePlaybackInfo(PlaybackInfo playbackInfo, int operationAcks, boolean positionDiscontinuity, int positionDiscontinuityReason) {
        ...
        if (this.pendingOperationAcks == 0) {
            if (playbackInfo.startPositionUs == C0615C.TIME_UNSET) {
    ...
    private long playbackInfoPositionUsToWindowPositionMs(long positionUs) {
        long positionMs = C0615C.usToMs(positionUs);
