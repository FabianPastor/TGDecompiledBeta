...
public abstract class SegmentDownloader<M, K> implements Downloader {
    ...
    public final synchronized void download(ProgressListener listener) throws IOException, InterruptedException {
        this.priorityTaskManager.add(C0605C.PRIORITY_DOWNLOAD);
        try {
            ...
            for (int i = 0; i < segments.size(); i++) {
                CacheUtil.cache(((Segment) segments.get(i)).dataSpec, this.cache, this.dataSource, buffer, this.priorityTaskManager, C0605C.PRIORITY_DOWNLOAD, cachingCounters, true);
                this.downloadedBytes += cachingCounters.newlyCachedBytes;
                this.downloadedSegments++;
                notifyListener(listener);
            }
            this.priorityTaskManager.remove(C0605C.PRIORITY_DOWNLOAD);
        } catch (Throwable th) {
            this.priorityTaskManager.remove(C0605C.PRIORITY_DOWNLOAD);
        }
    }
