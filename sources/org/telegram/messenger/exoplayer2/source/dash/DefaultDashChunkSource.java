...
public class DefaultDashChunkSource implements DashChunkSource {
    ...
    private long liveEdgeTimeUs = C0621C.TIME_UNSET;
    ...
    public void getNextChunk(MediaChunk previous, long playbackPositionUs, long loadPositionUs, ChunkHolder out) {
        if (this.fatalError == null) {
            ...
            long presentationPositionUs = (C0621C.msToUs(this.manifest.availabilityStartTimeMs) + C0621C.msToUs(this.manifest.getPeriod(this.periodIndex).startMs)) + loadPositionUs;
            if (this.playerTrackEmsgHandler == null || !this.playerTrackEmsgHandler.maybeRefreshManifestBeforeLoadingNextChunk(presentationPositionUs)) {
                ...
                if (availableSegmentCount == -1) {
                    long liveEdgeTimeInPeriodUs = (getNowUnixTimeUs() - C0621C.msToUs(this.manifest.availabilityStartTimeMs)) - C0621C.msToUs(this.manifest.getPeriod(this.periodIndex).startMs);
                    if (this.manifest.timeShiftBufferDepthMs != C0621C.TIME_UNSET) {
                        firstAvailableSegmentNum = Math.max(firstAvailableSegmentNum, representationHolder.getSegmentNum(liveEdgeTimeInPeriodUs - C0621C.msToUs(this.manifest.timeShiftBufferDepthMs)));
                    }
                ...
                out.chunk = newMediaChunk(representationHolder, this.dataSource, this.trackType, this.trackSelection.getSelectedFormat(), this.trackSelection.getSelectionReason(), this.trackSelection.getSelectionData(), segmentNum, (int) Math.min((long) this.maxSegmentsPerLoad, (lastAvailableSegmentNum - segmentNum) + 1), previous == null ? loadPositionUs : C0621C.TIME_UNSET);
            }
        }
    }
    ...
    private void updateLiveEdgeTimeUs(RepresentationHolder representationHolder, long lastAvailableSegmentNum) {
        this.liveEdgeTimeUs = this.manifest.dynamic ? representationHolder.getSegmentEndTimeUs(lastAvailableSegmentNum) : C0621C.TIME_UNSET;
    }
    ...
    private long resolveTimeToLiveEdgeUs(long playbackPositionUs) {
        boolean resolveTimeToLiveEdgePossible = this.manifest.dynamic && this.liveEdgeTimeUs != C0621C.TIME_UNSET;
