...
public class EncryptedFileInputStream extends FileInputStream {
    ...
    public EncryptedFileInputStream(File file, SecureDocumentKey secureDocumentKey) throws Exception {
        ...
        System.arraycopy(secureDocumentKey.file_iv, 0, this.f63iv, 0, this.f63iv.length);
    }
    ...
    public int read(byte[] b, int off, int len) throws IOException {
        if (this.currentMode == 1 && this.fileOffset == 0) {
            ...
            Utilities.aesCbcEncryptionByteArraySafe(b, this.key, this.f63iv, off, len, this.fileOffset, 0);
            this.fileOffset += 32;
        ...
        if (this.currentMode == 1) {
            ...
            Utilities.aesCtrDecryptionByteArray(b, this.key, this.f63iv, off, len, this.fileOffset);
        }
        this.fileOffset += len;
        return result;
    }
