...
public class SRPHelper {
    ...
    public static byte[] getX(byte[] passwordBytes, C0579xb6caa888 algo) {
    ...
    public static BigInteger getV(byte[] passwordBytes, C0579xb6caa888 algo) {
        BigInteger g = BigInteger.valueOf((long) algo.f49g);
        byte[] g_bytes = getBigIntegerBytes(g);
        return g.modPow(new BigInteger(1, getX(passwordBytes, algo)), new BigInteger(1, algo.f50p));
    }

    public static byte[] getVBytes(byte[] passwordBytes, C0579xb6caa888 algo) {
        if (Utilities.isGoodPrime(algo.f50p, algo.f49g)) {
            return getBigIntegerBytes(getV(passwordBytes, algo));
        }
        return null;
    }

    public static TL_inputCheckPasswordSRP startCheck(byte[] x_bytes, long srp_id, byte[] srp_B, C0579xb6caa888 algo) {
        if (x_bytes == null || srp_B == null || srp_B.length == 0 || !Utilities.isGoodPrime(algo.f50p, algo.f49g)) {
            return null;
        }
        BigInteger g = BigInteger.valueOf((long) algo.f49g);
        byte[] g_bytes = getBigIntegerBytes(g);
        BigInteger bigInteger = new BigInteger(1, algo.f50p);
        bigInteger = new BigInteger(1, Utilities.computeSHA256(algo.f50p, g_bytes));
        ...
        byte[] p_hash = Utilities.computeSHA256(algo.f50p);
