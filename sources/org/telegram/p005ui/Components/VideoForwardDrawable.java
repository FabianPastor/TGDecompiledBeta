...
public class VideoForwardDrawable extends Drawable {
    ...
    public VideoForwardDrawable() {
        ...
        for (int a = 0; a < playPath.length / 2; a++) {
            if (a == 0) {
                this.path1.moveTo((float) AndroidUtilities.m9dp((float) playPath[a * 2]), (float) AndroidUtilities.m9dp((float) playPath[(a * 2) + 1]));
            } else {
                this.path1.lineTo((float) AndroidUtilities.m9dp((float) playPath[a * 2]), (float) AndroidUtilities.m9dp((float) playPath[(a * 2) + 1]));
            }
        }
        this.path1.close();
    }
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.leftSide) {
            x -= (rect.width() / 4) - AndroidUtilities.m9dp(16.0f);
        } else {
            x += (rect.width() / 4) + AndroidUtilities.m9dp(16.0f);
        }
        canvas.save();
        ...
        canvas.drawCircle((float) (((this.leftSide ? -1 : 1) * (Math.max(rect.width(), rect.height()) / 4)) + x), (float) (AndroidUtilities.m9dp(16.0f) + y), (float) (Math.max(rect.width(), rect.height()) / 2), this.paint);
        canvas.restore();
        canvas.save();
        if (this.leftSide) {
        ...
        canvas.translate((float) AndroidUtilities.m9dp(18.0f), 0.0f);
