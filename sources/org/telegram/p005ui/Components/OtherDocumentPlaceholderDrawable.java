...
public class OtherDocumentPlaceholderDrawable extends RecyclableDrawable implements FileDownloadProgressListener {
    ...
    public OtherDocumentPlaceholderDrawable(Context context, View view, MessageObject messageObject) {
        docPaint.setTextSize((float) AndroidUtilities.m9dp(14.0f));
        namePaint.setTextSize((float) AndroidUtilities.m9dp(19.0f));
        sizePaint.setTextSize((float) AndroidUtilities.m9dp(15.0f));
        buttonPaint.setTextSize((float) AndroidUtilities.m9dp(15.0f));
        percentPaint.setTextSize((float) AndroidUtilities.m9dp(15.0f));
        openPaint.setTextSize((float) AndroidUtilities.m9dp(15.0f));
        ...
        if (document != null) {
            ...
            this.ext = idx == -1 ? TtmlNode.ANONYMOUS_REGION_ID : this.fileName.substring(idx + 1).toUpperCase();
            if (((int) Math.ceil((double) docPaint.measureText(this.ext))) > AndroidUtilities.m9dp(40.0f)) {
                this.ext = TextUtils.ellipsize(this.ext, docPaint, (float) AndroidUtilities.m9dp(40.0f), TruncateAt.END).toString();
            }
            ...
            if (((int) Math.ceil((double) namePaint.measureText(this.fileName))) > AndroidUtilities.m9dp(320.0f)) {
                this.fileName = TextUtils.ellipsize(this.fileName, namePaint, (float) AndroidUtilities.m9dp(320.0f), TruncateAt.END).toString();
            }
        }
        checkFileExist();
    }
    ...
    public void draw(Canvas canvas) {
        ...
        int y = (height - AndroidUtilities.m9dp(240.0f)) / 2;
        int x = (width - AndroidUtilities.m9dp(48.0f)) / 2;
        this.thumbDrawable.setBounds(x, y, AndroidUtilities.m9dp(48.0f) + x, AndroidUtilities.m9dp(48.0f) + y);
        this.thumbDrawable.draw(canvas);
        Canvas canvas2 = canvas;
        canvas2.drawText(this.ext, (float) ((width - ((int) Math.ceil((double) docPaint.measureText(this.ext)))) / 2), (float) (AndroidUtilities.m9dp(31.0f) + y), docPaint);
        canvas2 = canvas;
        canvas2.drawText(this.fileName, (float) ((width - ((int) Math.ceil((double) namePaint.measureText(this.fileName)))) / 2), (float) (AndroidUtilities.m9dp(96.0f) + y), namePaint);
        canvas2 = canvas;
        canvas2.drawText(this.fileSize, (float) ((width - ((int) Math.ceil((double) sizePaint.measureText(this.fileSize)))) / 2), (float) (AndroidUtilities.m9dp(125.0f) + y), sizePaint);
        if (this.loaded) {
            button = LocaleController.getString("OpenFile", CLASSNAMER.string.OpenFile);
            paint = openPaint;
            offsetY = 0;
        } else {
            if (this.loading) {
                button = LocaleController.getString("Cancel", CLASSNAMER.string.Cancel).toUpperCase();
            } else {
                button = LocaleController.getString("TapToDownload", CLASSNAMER.string.TapToDownload);
            }
            offsetY = AndroidUtilities.m9dp(28.0f);
            paint = buttonPaint;
        }
        canvas.drawText(button, (float) ((width - ((int) Math.ceil((double) paint.measureText(button)))) / 2), (float) ((AndroidUtilities.m9dp(235.0f) + y) + offsetY), paint);
        if (this.progressVisible) {
            if (this.progress != null) {
                canvas2 = canvas;
                canvas2.drawText(this.progress, (float) ((width - ((int) Math.ceil((double) percentPaint.measureText(this.progress)))) / 2), (float) (AndroidUtilities.m9dp(210.0f) + y), percentPaint);
            }
            x = (width - AndroidUtilities.m9dp(240.0f)) / 2;
            y += AndroidUtilities.m9dp(232.0f);
            progressPaint.setColor(-10327179);
            progressPaint.setAlpha((int) (255.0f * this.animatedAlphaValue));
            canvas.drawRect((float) (x + ((int) (((float) AndroidUtilities.m9dp(240.0f)) * this.animatedProgressValue))), (float) y, (float) (AndroidUtilities.m9dp(240.0f) + x), (float) (AndroidUtilities.m9dp(2.0f) + y), progressPaint);
            progressPaint.setColor(-1);
            progressPaint.setAlpha((int) (255.0f * this.animatedAlphaValue));
            Canvas canvas3 = canvas;
            canvas3.drawRect((float) x, (float) y, (((float) AndroidUtilities.m9dp(240.0f)) * this.animatedProgressValue) + ((float) x), (float) (AndroidUtilities.m9dp(2.0f) + y), progressPaint);
            updateAnimation();
        }
        canvas.restore();
    }
