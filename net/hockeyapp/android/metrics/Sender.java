...
public class Sender
{
  ...
  {
    return (200 <= paramInt) && (paramInt <= 203);
  }
  ...
  protected void onUnexpected(HttpURLConnection paramHttpURLConnection, int paramInt, StringBuilder paramStringBuilder)
  {
    ...
    HockeyLog.error("HockeyApp-Metrics", str);
  ...
  protected void triggerSending()
  {
    if (requestCount() < 10) {
      ...
      {
        HockeyLog.error("Could not send events. Executor rejected async task.", localRejectedExecutionException);
        return;
      }
    }
