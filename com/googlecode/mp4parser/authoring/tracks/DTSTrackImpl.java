...
public class DTSTrackImpl extends AbstractTrack {
    ...
    private boolean readVariables() throws IOException {
        ...
        if (testHeader1 == NUM && testHeader2 == NUM) {
            while (true) {
                if (!(testHeader1 == NUM && testHeader2 == NUM) && bb.remaining() > 100) {
                    int size = (int) bb.getLong();
                    if (testHeader1 == NUM && testHeader2 == NUM) {
                        parseDtshdhdr(size, bb);
                    } else if (testHeader1 == NUM && testHeader2 == NUM) {
                        if (!parseCoressmd(size, bb)) {
                            return false;
                        }
                    } else if (testHeader1 == NUM && testHeader2 == 759710802) {
