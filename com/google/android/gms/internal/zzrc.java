...
public final class zzrc extends Fragment implements zzrb {
    ...
    public static zzrc zzt(Activity activity) {
        WeakReference weakReference = (WeakReference) yZ.get(activity);
        if (weakReference != null) {
            zzrc com_google_android_gms_internal_zzrc = (zzrc) weakReference.get();
        }
        try {
            com_google_android_gms_internal_zzrc = (zzrc) activity.getFragmentManager().findFragmentByTag("LifecycleFragmentImpl");
            if (com_google_android_gms_internal_zzrc == null || com_google_android_gms_internal_zzrc.isRemoving()) {
                com_google_android_gms_internal_zzrc = new zzrc();
                activity.getFragmentManager().beginTransaction().add(com_google_android_gms_internal_zzrc, "LifecycleFragmentImpl").commitAllowingStateLoss();
            }
            yZ.put(activity, new WeakReference(com_google_android_gms_internal_zzrc));
