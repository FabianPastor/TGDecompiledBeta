...
public final class zzsd extends Fragment implements zzrp {
    ...
    public static zzsd zza(FragmentActivity fragmentActivity) {
        WeakReference weakReference = (WeakReference) Bg.get(fragmentActivity);
        if (weakReference != null) {
            zzsd com_google_android_gms_internal_zzsd = (zzsd) weakReference.get();
        }
        try {
            com_google_android_gms_internal_zzsd = (zzsd) fragmentActivity.getSupportFragmentManager().findFragmentByTag("SupportLifecycleFragmentImpl");
            if (com_google_android_gms_internal_zzsd == null || com_google_android_gms_internal_zzsd.isRemoving()) {
                com_google_android_gms_internal_zzsd = new zzsd();
                fragmentActivity.getSupportFragmentManager().beginTransaction().add(com_google_android_gms_internal_zzsd, "SupportLifecycleFragmentImpl").commitAllowingStateLoss();
            }
            Bg.put(fragmentActivity, new WeakReference(com_google_android_gms_internal_zzsd));
            return com_google_android_gms_internal_zzsd;
        } catch (Throwable e) {
    ...
    private void zzb(final String str, @NonNull final zzro com_google_android_gms_internal_zzro) {
        if (this.zzbtt > 0) {
            new Handler(Looper.getMainLooper()).post(new Runnable(this) {
                ...
                public void run() {
                    if (this.Bu.zzbtt >= 1) {
                        com_google_android_gms_internal_zzro.onCreate(this.Bu.Bi != null ? this.Bu.Bi.getBundle(str) : null);
                    }
                    if (this.Bu.zzbtt >= 2) {
